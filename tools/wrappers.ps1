...
Remove-Item Function:\Get-FirstProp -ErrorAction SilentlyContinue
...
Remove-Item Function:\Normalize-JarvisPatch -ErrorAction SilentlyContinue
function Normalize-JarvisPatch {
  param([Parameter(Mandatory)]$Patch)

  # 1) Konverter til objekt med .actions
  $obj = $Patch
  if ($Patch -is [string]) {
    $obj = ConvertFrom-JarvisPatchJson -Json $Patch  # bruger eksisterende helper
  }
  ...
  foreach ($raw in $obj.actions) {
    ...
    # 2) Strings -> simpel parsing (uden komplekse regex’er)
    if ($a -is [string]) {
      $s   = $a.Trim()
      $op  = $null
      $pth = $null
      ...
      # format: "op path: content"
      $colon = $s.IndexOf(':')
      if ($colon -gt 0) {
        $lhs = $s.Substring(0, $colon).Trim()
        $rhs = $s.Substring($colon + 1).Trim()
        $tkn = $lhs -split '\s+', 2
        if ($tkn.Length -ge 2) { $op = $tkn[0]; $pth = $tkn[1]; $txt = $rhs }
      }
      else {
        # format: "op path [content...]"
        $tkn = $s -split '\s+', 3
        if ($tkn.Length -ge 2) {
          $op  = $tkn[0]
          $pth = $tkn[1]
          $txt = if ($tkn.Length -ge 3) { $tkn[2] } else { '' }
        }
      }
  ...
  $a = [pscustomobject]@{
    ...
    if ($null -ne $op)  { $op  = [string]$op }  else { $op  = $null }
    if ($null -ne $pth) { $pth = [string]$pth } else { $pth = $null }
    if ($null -ne $txt) { $txt = [string]$txt } else { $txt = '' }

    if (-not $op -and $pth) { $op = 'write' }

    switch (($op -as [string]).ToLowerInvariant()) {
      'create'       { $op = 'write' }
      'add'          { $op = 'write' }
      'put'          { $op = 'write' }
      'writefile'    { $op = 'write' }
      'append_text'  { $op = 'append' }
      'appendline'   { $op = 'append' }
      'mkdir'        { $op = 'ensureDir' }
      'mkpath'       { $op = 'ensureDir' }
      'rm'           { $op = 'remove' }
      'del'          { $op = 'remove' }
      default        { } # behold write/append/ensureDir/ensureLine/remove hvis allerede sat
    }
    ...
    $normPath = $null
    if ($pth) {
      try { $normPath = Jarvis-NormalizeRelPath -Path $pth } catch { continue }
    }

    $out += [pscustomobject]@{ op=$op; path=$normPath; content=$txt }
  }
...
Remove-Item Function:\Jarvis-ValidatePatch -ErrorAction SilentlyContinue
function Jarvis-ValidatePatch {
  param([Parameter(Mandatory)]$Patch)
...
function Normalize-JarvisPatch {
  ...
  $obj = $Patch
  if ($Patch -is [string]) {
    $obj = ConvertFrom-JarvisPatchJson -Json $Patch  # bruger din eksisterende helper
  }
...
# --- ConvertFrom-JarvisPatchJson (robust) ---
Remove-Item Function:\ConvertFrom-JarvisPatchJson -ErrorAction SilentlyContinue
function ConvertFrom-JarvisPatchJson {
  param([Parameter(Mandatory)]$Json)

  if ($Json -isnot [string]) { return $Json }
  $text = [string]$Json

  # strip ```json fences hvis de findes
  ...
  # fallback-format:  write <path>: <content>
  $m = [regex]::Match($text, '^(?is)\s*(\w+)\s+([^\s:]+)\s*:\s*(.+?)\s*$')
  if ($m.Success) {
    $op  = $m.Groups[1].Value.ToLowerInvariant()
    if ($op -in @('create','add','put')) { $op = 'write' }
    $pth = $m.Groups[2].Value
    $txt = $m.Groups[3].Value
    return [pscustomobject]@{
      actions = @([pscustomobject]@{ op = $op; path = $pth; content = $txt })
    }
  }

  throw "Patch mangler 'actions' (hverken JSON eller 'op path: content')."
}
# --- end ConvertFrom-JarvisPatchJson ---
