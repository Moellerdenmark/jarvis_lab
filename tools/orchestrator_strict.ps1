...
function Run-Proc([string]$exe,[string[]]$args,[int]$timeoutSec,[string]$name){
  $logs = Join-Path (Join-Path $Repo 'tools') 'logs'
  ...
  if(-not $p.WaitForExit($timeoutSec*1000)){
    ...
    throw "$name timeout efter ${timeoutSec}s (se $out / $err)"
  }
...
if(Test-Path $lock){ throw "Allerede i gang (.autopilot.lock findes)" }
...
try{
  ...
  $task = Get-ChildItem $qPath -File -Filter *.md | Sort-Object Name | Select -First 1
  ...
  if($DryRun){
    ...
    if(Test-Path $aiLoop){
      ...
      $inbox = Split-Path -Parent $task.FullName
      $tmp   = Join-Path $inbox '_tmp_hidden'
      New-Item -ItemType Directory -Force -Path $tmp | Out-Null
      Get-ChildItem $inbox -File -Filter *.md | ? { $_.FullName -ne $task.FullName } | % { Move-Item $_.FullName $tmp -Force }
      try{
        Run-Proc $PSCmd @('-NoProfile','-ExecutionPolicy','Bypass','-File',$orch) $AiStepTimeoutSec 'orch'
      } finally {
        Get-ChildItem $tmp -File -Filter *.md -EA SilentlyContinue | % { Move-Item $_.FullName $inbox -Force }
        Remove-Item $tmp -Recurse -Force -EA SilentlyContinue
      }
    } else {
      throw "Mangler både tools\ai_loop.ps1 og tools\orchestrator.ps1"
    }
  }

  # hard gate tests
  $tests = @()
  $vs = Join-Path $Repo 'tools\tests\voice_smoke.ps1'; if(Test-Path $vs){ $tests += $vs }
  $sc = Join-Path $Repo 'tools\selfcheck.ps1';        if(Test-Path $sc){ $tests += $sc }
  if($tests.Count -eq 0){
    Say "ADVARSEL: Ingen lokale tests fundet – overvej at tilføje voice_smoke.ps1/selfcheck.ps1"
  } else {
    foreach($t in $tests){
      Say ("Kører test: {0}" -f $t)
      Run-Proc $PSCmd @('-NoProfile','-ExecutionPolicy','Bypass','-File',$t) 300 ('test_'+[IO.Path]::GetFileNameWithoutExtension($t))
    }
  }
  ...
  $title = "[Jarvis] $($task.BaseName)"
  $body  = "Auto-created by orchestrator_strict on $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
  Say "Opretter PR…"
  ...
  do{
    $merged = (gh pr view $pr --json merged -q ".merged" 2>$null)
    if($merged -eq 'true'){ break }
  ...
  $last = (Test-Path $marker) ? (Get-Content $marker -Raw) : (git rev-list --max-parents=0 HEAD | Select -First 1)
  $head = (git rev-parse HEAD).Trim()
  $files = (git diff --name-only $last $head | Out-String).Trim().Split("`n") |
           ? { $_ } | ? { $_ -notmatch '(^\.git|^\.github|^ai_inbox|^ai_tasks|\.md$)' }
  if(-not $files -or $files.Count -eq 0){
    Say "Ingen promoverbare ændringer."
    $head | Set-Content $marker
  } else {
    foreach($rel in $files){
      ...
      Say ("Promoted: {0}" -f $rel)
    }
    $head | Set-Content $marker
    Say ("Promotion complete → {0}" -f $Core)
  }
